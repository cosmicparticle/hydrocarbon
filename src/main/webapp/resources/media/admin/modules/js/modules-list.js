define(function(require, exports, module) {
	...
	exports.init = function($page, moduleName, menuId, menuTitle, pageInfo,
			sessionExportStatus) {
		var $exportAll = $('#export-all', $page), $exportCur = $(
				'#export-current-page', $page), $exportMsg = $('#export-msg',
				$page), $withDetail = $('#with-detail', $page), $msg = $('p',
				$exportMsg);

		$exportAll.change(function(e, flag) {
			var checked = $exportAll.prop('checked');
			if (!flag) {
				$exportCur.prop('checked', !checked)
						.trigger('change', [ true ]);
			}
		});
		...
		var $btnExport = $('#do-export', $page), $btnBreak = $('#do-break',
				$page), $btnDownload = $('#do-download', $page);
		// 页面一开始加载时的初始化表单参数

		var initParam = {};
		Utils.botByDom($page.getLocatePage().getContent(), 'cpf-page-inited',
				function() {
					var $form = $('form', $page), formData = new FormData(
							$form[0]);
		...
		var subscriber = exportHandler.addSubscriber({
			...
			whenCompleted : function(res) {
				if (res.uuid) {
					$msg.text('导出完成');
					$btnDownload.removeAttr('disabled').off('click').click(
							function() {
								Ajax.download('admin/modules/export/download/'
										+ res.uuid);
							}).show();
		...
		function progressHandler(progress, res) {
			...
			$exportProgress.find('.progress-text').text(percent).css('left',
					(parseFloat(progressText) - 3) + '%');
			$exportProgress.find('.progress-bar')
		...
		if (sessionExportStatus.uuid) {
			if (exportHandler.getStatus() != 'working') {
				...
				Ajax
						.ajax(
								'admin/modules/export/work/'
										+ sessionExportStatus.uuid)
						.done(
								function(work) {
									if (work.menuId == menuId) {
										if (work.scope === 'current') {
											$exportCur.prop('checked', true)
													.trigger('change');
										} else if (work.scope === 'all') {
											$('#export-range-start', $page)
													.val(work.rangeStart);
											$('#export-range-end', $page).val(
													work.rangeEnd);
											$exportAll.prop('checked', true)
													.trigger('change');
										}
										$withDetail.prop('checked',
												work.withDetail == 'true')
												.trigger('change');
										// menu对应
										...
										// menu不对应
										// 创建空回调的订阅者
										var workingSubscriber = exportHandler
												.addSubscriber({
													startupURL : 'admin/modules/export/start/'
															+ work.menuId,
													progressURL : 'admin/modules/export/status',
													data : {
		...
		$btnExport
				.click(function() {
					var scope = $exportAll.prop('checked') ? 'all' : $exportCur
							.prop('checked') ? 'current' : null;
					if (scope) {
						var rangeStart = scope == 'all'
								&& $('#export-range-start', $page).val()
								|| undefined, rangeEnd = scope == 'all'
								&& $('#export-range-end', $page).val()
								|| undefined, withDetail = $withDetail
								.prop('checked');
						subscriber.starts({
							scope : scope,
						...
						function(callbacks) {
							callbacks
									.add(function(e) {
										if (exportHandler
												&& exportHandler
														.getWorkingSubscriber() == subscriber) {
											if (e.eventName === 'beforeClose') {
												require('dialog')
														.notice(
																'当前页面正在执行导出，需要等待导出结束后，才允许关闭',
																'error');
											} else if (e.eventName === 'beforeLoad') {
												require('dialog')
														.notice(
																'当前页面正在执行导出，需要等待导出结束后，才允许刷新',
																'error');
											}
											e.stopDefault();
										} else {
		...
		function startPolling() {
			...
			$exportProgress.find('.progress-bar').attr('aria-valuenow', 0).css(
					'width', 0);
			$exportProgress.show();
			$btnExport.hide();
		...
		function toggleWait(working) {
			...
			$menuLink.text(working.getData('menuTitle')).attr('href',
					'admin/modules/curd/list/' + menuId).attr('target',
					'entity_list_' + menuId).addClass('tab');
		...
		var $table = $('table', $page).on(
				'row-selected-change',
				function(e, $checkedRows) {
					if ($checkedRows.length === 0) {
						...
						if ($checkedRows.length > 1) {
							$actionButtons.filter('[data-multiple="0"]').attr(
									'disabled', 'disabled');
							$jumpButtons.filter('[data-multiple="0"]').attr(
									'disabled', 'disabled');
						}
					}
				});
		function doAction(actionId, actionTitle) {
			...
			default:
				...
				confirm = '确定执行操作【' + actionTitle + '】？共选择了' + codes.length
						+ '项';
			}
			if (actionId) {
