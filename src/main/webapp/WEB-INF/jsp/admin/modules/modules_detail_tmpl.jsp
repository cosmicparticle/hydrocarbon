...
<link type="text/css" rel="stylesheet"
	href="media/admin/modules/css/modules-detail-tmpl.css" />
<title>${module.title}-${entity.title }-详情</title>
<div class="entity-detail-tmpl entity-detail-page"
	id="${moduke.key }-detail-tmpl-${entity.code }-${RES_STAMP}">
	<div class="page-header">
		...
		<div class="header-buttons">
			...
			<c:if test="${hasHistory == true }">
				<a href="page:#timeline-area.toggle" title="查看历史"
					class="toggle-timeline btn-toggle"> <i
					class="iconfont icon-historyrecord"></i>
				</a>
			</c:if>
		...
		<div class="col-lg-offset-1 col-lg-10">
			<form class="form-horizontal group-container">
				...
				<c:forEach var="tmplGroup" items="${dtmpl.groups }">
					<div class="widget field-group"  field-group-id="${tmplGroup.id }">
						<div class="widget-header">
							<span class="widget-caption"> <span class="group-title">${tmplGroup.title }</span>
							</span>
							<c:if
								test="${tmplGroup.isArray == 1 && !empty entity.arrayMap[tmplGroup.composite.name]}">
								<div class="widget-buttons keyword-search-container">
									<span class="input-icon"> <input type="text"
										class="form-control input-xs" placeholder="关键字"> <i
										...
										<div
											...
											<div class="field-value">
												<span class="field-view"
													refgroupid="${tmplField.refGroupId }"
													menuid="${tmplField.refGroupId!=null ? menu.id:'' }"
													field-type="${tmplField.type }">${tmplField.fieldAvailable? entity.smap[tmplField.fieldName]: '' }</span>
											</div>
										</div>
									</c:forEach>
								</c:when>
										...
										<table class="table table-striped table-bordered table-hover">
											<thead>
												<tr class="title-row">
													...
													<c:forEach var="field" items="${tmplGroup.fields }">
														<th
															class="${field.fieldAvailable? field.type == 'file'? '': 'sorting': 'field-unavailable'}"
											...
											<tbody>
												<c:forEach var="entityItem" varStatus="i"
													items="${entity.arrayMap[tmplGroup.composite.name] }">
													<tr class="value-row" origin-order="${i.index }">
													<td>${i.index + 1 }</td>
														<c:if test="${tmplGroup.relationSubdomain != null }">
															<c:set var="relationName"
																value="${tmplGroup.composite.name }.$$label$$" />
															<td>${entityItem.smap[relationName] } <input
														...
														<c:forEach var="field" items="${tmplGroup.fields }">
															<td
																class="${field.fieldAvailable? '': 'field-unavailable'}"
																field-type="${field.type }">${field.fieldAvailable? entityItem.smap[field.fieldName] : ''}</td>
														</c:forEach>
														<td><c:if
																test="${tmplGroup.rabcTemplateGroupId != null && tmplGroup.rabcUndetailable != 1 }">
	...
	<div id="timeline-area" class="blur-hidden" style="display: none;">
		<div class="timeline-wrapper">
			<div class="VivaTimeline">
				<dl>
					<dt>
						<a href="#" class="show-more-history">查看更多</a>
					</dt>
				</dl>
			</div>
		</div>
